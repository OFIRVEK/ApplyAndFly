...
const CLASSIFICATION_SCHEMA = {
  ...
  schema: {
    ...
    properties: {
      ...
      suspicionType: {
        type: ["string", "null"],
        enum: ["phishing_fraud", "prank_joke", null],
        description: "Evaluate this independently of eventType above — a scam can be written to look exactly like a normal confirmation/interview/offer. phishing_fraud = signs of a scam or phishing attempt impersonating a real company or recruiter: the sender's email domain doesn't plausibly belong to the claimed company, it asks for payment, bank/card details, a government ID number, or your password, it asks you to click a link to 'verify' or 'confirm' your account/credentials, it promises an unrealistic instant job offer with no interview or screening, or it asks you to pay upfront for training/equipment/a background check. prank_joke = the email is clearly not a genuine business communication at all (a joke from a friend, meme content, an obvious test message) with no real deceptive intent. null = a normal, good-faith email, even if its eventType ends up non_job or uncertain.",
      },
      suspicionReason: { type: ["string", "null"], description: "One short sentence naming the specific red flag that justified suspicionType. Null when suspicionType is null." },
    ...
    required: ["eventType", "company", "position", "status", "recruiterName", "location", "referredBy", "suspicionType", "suspicionReason", "confidence", "evidenceQuotes", "recommendedAction"],
...
For "suspicionType" and "suspicionReason": check this REGARDLESS of what eventType you chose above — a scam email can be written to imitate a normal confirmation, interview invite, or offer perfectly. Compare the sender's email address/domain against the company the email claims to be from — a real company's HR/recruiting email essentially never comes from an unrelated free/generic domain or a domain that has nothing to do with the company name (a well-known ATS platform sending on the company's behalf, e.g. Greenhouse/Lever/Workday, is normal and NOT itself a red flag). Watch for: a request for payment, bank details, a credit card, or a government ID number; a request to click a link and "verify"/"confirm" your account, password, or personal details; an unrealistic instant job offer with no interview or screening process at all; a request to pay upfront for training, equipment, or a background check before starting. Any of those is "phishing_fraud". If instead the email is obviously not a real business communication at all — a joke from a friend, meme text, a one-line test message — with no deceptive intent, use "prank_joke". Do not over-flag: a normal, professional email that just happens to be a rejection, a generic ATS-templated confirmation, or an unrelated (non_job) email is NOT suspicious — leave both fields null unless there's a genuine, specific red flag you can name in suspicionReason.
